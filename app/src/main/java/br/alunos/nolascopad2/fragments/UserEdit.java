 ...
 * {@link UserEdit.OnFragmentInteractionListener} interface
...
public class UserEdit extends Fragment {
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
        ...
        final EditText nometext;
        final ImageView backbtn;
        Button edituser;
        final UserDAO userDAO = new UserDAO(getActivity().getApplicationContext());
        final User user = userDAO.getUserFromDB(getArguments().getInt("CurrentUser"));
        ...
        edituser.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                if (tempuser.nome == "" || tempuser.email == "") {
                    Toast.makeText(getActivity().getApplicationContext(), "Campos foram deixados em branco", Toast.LENGTH_LONG).show();
                } else {
                    if (user.nome.length() >= 5 || user.email.length() >= 12) {
                        if (user.email.contains("@") && (user.email.contains(".com") || user.email.contains(".br"))) {

                            if (userDAO.getAllUsersExceptLogged(user.id).isEmpty()) {

                                userDAO.editUser(tempuser);
                                getActivity().finish();

                            } else Toast.makeText(getActivity().getApplicationContext(), "Email já existente", Toast.LENGTH_LONG).show();

                        }else Toast.makeText(getActivity().getApplicationContext(), "Email inválido", Toast.LENGTH_LONG).show();

                    }else Toast.makeText(getActivity().getApplicationContext(), "Campos inválidos", Toast.LENGTH_LONG).show();
